...
import { GETALL_ALLERGY_APPOINTMENT, HOSP_GETALL_ALLERGY_APPOINTMENT } from '../constants/APIUrl';
...
let GETALL_ALLERGY_APPOINTMENT_URL = '';
class AppointmentsByAllergy extends Component {
    constructor(props) {
        super(props);
        this.state = { AccessToken: '', Userid: '', allergyId: '', appointmentallergyResp: [], isLoading: true, refreshing: false };
        this.props.navigation.addListener(
            'didFocus',
            async () => {
                GETALL_ALLERGY_APPOINTMENT_URL = GETALL_ALLERGY_APPOINTMENT;
                ...
                if (USER_DATA.hasOwnProperty('Hospital') && USER_DATA.Hospital.AppointmentLogin == false) {
                    ...
                    GETALL_ALLERGY_APPOINTMENT_URL = USER_DATA.ServiceURL + HOSP_GETALL_ALLERGY_APPOINTMENT;
                }
                this.setState({
    ...
    /*****************************************Get BillList **************************************************************/
    getAppointmentByAllergy = () => {
        fetch(`${GETALL_ALLERGY_APPOINTMENT_URL}?allergyId=${this.state.allergyId}`, {
            method: 'GET',
            headers: {
            ...
            this.setState({
                isLoading: false,
                refreshing: false,
                appointmentallergyResp: resp.responseData,
            });
        }).catch((error) => {
            this.setState({
    ...
    updateState(data) {
        this.setState({ appointmentallergyResp: data.FilteredData });
    }
    render() {
        Moment.locale('en');
                    ...
                    <ScrollView
                        refreshControl={
                            <RefreshControl
                                ...
                                onRefresh={this.getAppointmentByAllergy}
                            />
                        }
                    >
                        <View style={{ flex: 1, marginTop: 5 }}>
                            {this.state.appointmentallergyResp.map(data => (
                                <View style={styles.card} >
                                    <View key={data.allergyId} style={{ marginRight: 5, flex: 1 }} >
                            ...
                            {this.state.appointmentallergyResp.length == 0 ?
...
export default AppointmentsByAllergy;
