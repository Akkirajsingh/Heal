...
import { Image, ScrollView, StyleSheet, Text, TouchableOpacity, TextInput, View, NetInfo, AsyncStorage, ToastAndroid, DatePickerAndroid, Modal, Dimensions, Animated, KeyboardAvoidingView, ActivityIndicator, Alert } from 'react-native';
...
class AppointmentsStatic extends Component {
    constructor(props) {
        super(props);
        const a = new Date();
        this.state = {
            ...
            physicianData: [], DepartSelectedItems: '', qualification: '', experience: '', PhysicianName: '', visitPurpose: '', visitPurposeItems: [], modalVisible: false, AppointmentDateTime: ''
            ...
            isHospitalDataLoading: false, healId: '', visitPurposeItemsLoading: false, DepartmentItemsLoading: false, ServiceURL: '', HospName: '', ACCESS_TOKEN: '', physicianDataLoading: false, VisitNotes: ''
        };
    }
    ...
    physicianData = () => {
        ...
        fetch(`${this.state.HOSP_USER_DATA.ServiceURL}api/PhysicianService/Physicians?practiceLocationId=${this.state.HospSelectedItems}&SpecialityId=${this.state.DepartSelectedItems}`, {
            method: 'GET',
            headers: {
                Authorization: `Bearer ${this.state.HOSP_USER_DATA.ACCESS_TOKEN}`,
                'content-Type': 'application/x-www-form-urlencoded',
            ...
            .then((responseJson) => {
                if (responseJson.responseData.length == 0) {
                    ...
                    this.setState({ isLoading: false, refreshing: false, physicianData: responseJson.responseData, physicianDataLoading: false });
                    return;
                }
                this.setState({
    ...
    saveAppointment = () => {
        ...
        const AppointmentData = {
            ...
            AppointmentType: this.state.visitPurpose,
                        ...
                        <View style={{ flex: 1, paddingTop: 14 }}>
                            <View style={{ paddingLeft: 15, paddingRight: 15 }}>
                                <Dropdown
                                    ...
                                    data={this.state.visitPurposeItems}
                                    textColor='#8d9393'
                                    labelHeight={6}
                                    labelFontSize={17}
                                    fontSize={16}
                                    selectedItemColor='#41b4af'
                                    onChangeText={(val) => this.changeVisitStatus(val)}
                                    value={this.state.visitPurpose}
                                    containerStyle={{ marginBottom: 8, marginTop: 8 }}

                                />
                                {this.state.visitPurposeItemsLoading ? <ActivityIndicator style={{ flex: 1, flexDirection: "row", justifyContent: "flex-end", alignItems: "flex-end", paddingRight: 5, marginRight: 20, marginTop: -40 }} /> : undefined}
                                <Text style={{ marginBottom: 8, marginTop: 8, fontSize: 17 }}>Notes</Text>
                                <TextInput
                                    multiline={true}
                                    numberOfLines={5}
                                    placeholder={''}
                                    ...
                                    onChangeText={(VisitNotes) => this.setState({ VisitNotes })}
                            ...
                            <View style={{ flex: 1, flexDirection: "column", justifyContent: "flex-end", marginTop: 20, alignItems: "flex-end", marginRight: 15 }} >
                                <TouchableOpacity style={styles.consultButton} onPress={() => {
                                    if (Utility.IsNullOrEmpty(this.state.VisitNotes) || Utility.IsNullOrEmpty(this.state.visitPurpose)) {
                                        ToastAndroid.showWithGravity(
                                            Utility.IsNullOrEmpty(this.state.VisitNotes) && Utility.IsNullOrEmpty(this.state.visitPurpose) ? "Purpose of visit and Notes is mandatory" : Utility.IsNullOrEmpty(this.state.VisitNotes) ? "Notes is mandatory" : "Purpose of visit is mandatory",
                                            ToastAndroid.SHORT,
                                            ToastAndroid.CENTER,
                                        ); return;
                                    } this.setState({ page: 2 })
                                }}>
                                    <Text style={styles.consultButtonText}>Next</Text>
                                </TouchableOpacity>
                            </View>
                ...
                {this.state.page === 2 ?
                    <ScrollView>
                        <View style={{ flex: 1, paddingTop: 14 }}>
                            ...
                            {!Utility.IsNullOrEmpty(this.state.DepartSelectedItems) && this.state.physicianData.length > 0 ?
                            ...
                            {!Utility.IsNullOrEmpty(this.state.DepartSelectedItems) && this.state.physicianData.map(data => (
                                <View>
                                    ...
                                    <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 15, borderBottomColor: '#9C9C9E', borderBottomWidth: 0.3 }}>
                                        ...
                                        <View>
                                            <TouchableOpacity style={styles.consultButton} onPress={() => { this.setState({ page: 3 }, function () { this.physicianAvailableData(data); }); }}>
...
export default AppointmentsStatic;