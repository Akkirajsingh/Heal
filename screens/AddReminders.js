...
import { TextInput, ScrollView, ActivityIndicator, StyleSheet, Text, Platform, AlertIOS, TouchableOpacity, View, NetInfo, ToastAndroid, AsyncStorage } from 'react-native';
...
import { SET_REMINDER, HOSP_SET_REMINDER } from '../constants/APIUrl';
import { REMINDER_SET_SUCCESS_MSG } from '../constants/Messages';
import { COMMON_ERROR } from '../constants/ErrorMessage';
import aPIStatusInfo from '../components/ErrorHandler';

let SET_REMINDER_URL = '';

let CONNECTION_STATUS = false;

const radio_props = [
    { label: 'Portal', value: 3 }, { label: 'Text Message', value: 1 }, { label: 'Email', value: 2 }
];
class AddReminders extends Component {
    constructor(props) {
        ...
        this.state = {
            loadingMsg: 'Loading Medical Problems....', todayDate: today, Remindervalue: '', userid: '', AccessToken: '', AccountId: '', emailId: '', reminderType: 3, mobilenumber: '', reminderDate: '', contactNo: '', isLoading: false, selectedHours: 0, selectedMinutes: 0, immunizationId: '', description: ''
        };
    }
    ...
    setReminder = () => {
        ...
        const ReminderService = {
            ...
            RemindMeIn: 0,
            ReminderDate: `${this.state.reminderDate} ${this.state.selectedHours}:${this.state.selectedMinutes}`,
            ReminderFor: 'General',
            ReminderItemId: '00000000-0000-0000-0000-000000000000',
            ReminderId: 0,
            ReminderType: this.state.reminderType,
            Subject: 'Remainder from Heal'
        };
        console.log('Reminder/Service', ReminderService);
        fetch(SET_REMINDER_URL, {
            method: 'POST',
            headers: {
    ...
    changereminderType = (value) => {
        console.log("changereminderType", value)
        this.setState({
            reminderType: value,
        });
    };
    /**********************************  Reminder Email Or Mobile View ************************************************************************/
    reminderEmail = () => (<View style={{ borderBottomWidth: 0.8, borderBottomColor: '#8d9393', }}>
    ...
    reminderMobile = () => (<View style={{ borderBottomWidth: 0.8, borderBottomColor: '#8d9393', }}>
        <View style={{ flexDirection: 'row', backgroundColor: '#f3f6fb', borderColor: '#746E6E', borderWidth: 0.2, paddingTop: 4, paddingBottom: 4 }}>
            <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <View style={{ borderBottomWidth: 0.8, borderBottomColor: '#8d9393', }}>
                            ...
                            <View style={styles.reminderContainer}>
                                <RadioForm
                                    ...
                                    labelStyle={{ fontSize: 16, paddingRight: 20, marginTop: -3 }}
                                    onPress={(value, index, data) => this.changereminderType(value, index, data)}
                                />
                            </View>
                        </View>

                        {this.state.reminderType == 2 ? this.reminderEmail() : (this.state.reminderType == 1 ? this.reminderMobile() : <Text />)}
                        <View style={{ borderBottomWidth: 0.8, borderBottomColor: '#8d9393', }}>
                            <View style={styles.reminderBox}>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
