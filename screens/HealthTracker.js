...
import { HEALTH_TRACKER_API, HOSP_HEALTH_TRACKER_API } from '../constants/APIUrl';
import { HEALTH_TRACKER_ADDED_SUCCESS_MSG } from '../constants/Messages';
...
class HealthTracker extends Component {
    constructor(props) {
        ...
        Moment.locale('en');
        this.state = {
            AccessToken: '', Userid: '', heightDate: todayDate, weightDate: todayDate, tempDate: todayDate, bloodPressureDate: todayDate, bloodSugarDate: todayDate, pulseRateDate: todayDate, PrandialbloodSugarDate: todayDate, Height: '', Weight: '', PulseRate: '', Temperature: '', BloodPressure: '', BloodSugar: '', PrandialbloodSugar: '',
            heightUnit: '', weightUnit: '', tempUnit: ''
        };
    }
    ...
    saveHealthTracker = () => {
        ...
        // } 
        let { Height, Weight, PulseRate, Temperature, BloodPressure, BloodSugar,PrandialbloodSugar  } = this.state;
        if(Height == '' || Weight == '' || Temperature == '' || PulseRate == '' || BloodPressure == '' || BloodSugar == '' || PrandialbloodSugar == '' ){
            ToastAndroid.showWithGravity(
        ...
        .then((response) => response.json()).then((res) => {
            if (res.statusCode == 200) {
                ToastAndroid.show(HEALTH_TRACKER_ADDED_SUCCESS_MSG, ToastAndroid.SHORT);
                this.setState({
                    isLoading: false,
                }, function () {
            ...
            <CommonView AddHealthTracker>
                <View style={{ flex: 1, paddingLeft: 12, paddingRight: 12 }}>
                    <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps='always'>
                        ...
                        <View style={{ flexDirection: 'row', borderBottomColor: '#8d9393', marginTop:5, borderBottomWidth: 0.3, alignItems: 'center', paddingBottom: 3 }}>
                        ...
                        <View style={{ alignItems: 'center', justifyContent: 'center', paddingTop: 7, paddingBottom: 12 }}>
                            <TouchableOpacity style={styles.cusButtonLargeGreen1} onPress={() => { this.saveHealthTracker(); }} >
...
const styles = StyleSheet.create({
    inputField: {
        height: 30, width: '100%', marginTop: 10, marginBottom: 15, flexDirection: 'row', borderBottomWidth: 0.4, borderBottomColor: 'grey', color: '#CBCACA'
    },
    boxTextCss: {
